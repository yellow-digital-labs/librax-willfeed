                ...
                var dt_user = dt_user_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        render: function render(data, type, full, meta) {
                            return '<div class="d-inline-block text-nowrap">' + "<a href=\"/blogs/edit/" + full['id'] + "\" class=\"btn btn-sm btn-icon\" data-id=\"".concat(full['id'], "\"><i class=\"ti ti-edit\"></i></a>") + "<button class=\"btn btn-sm btn-icon delete-record\" data-id=\"".concat(full['id'], "\"><i class=\"ti ti-trash\"></i></button>") + '<div class="dropdown-menu dropdown-menu-end m-0">' + '<a href="' + userView + '" class="dropdown-item">View</a>' + '<a href="javascript:;" class="dropdown-item">Suspend</a>' + '</div>' + '</div>';
                        }
                    }, {
                        // name
                        targets: 0,
                        ...
                        render: function render(data, type, full, meta) {
                            var $name = full['blog_name'] ? full['blog_name'] : '';
                            return '<span class="user-blog_name">' + $name + '</span>';
                        }
                    }, {
                        // active
                        targets: 1,
                        ...
                        responsivePriority: 4,
                        render: function render(data, type, full, meta) {
                            var $active = full['status'] ? full['status'] : '';
                            return '<span class="badge text-uppercase bg-label-'+($active=='active'?'success':'danger')+'">' + $active + '</span>';
                        }
                    }],
                    ...
                    buttons: [{
                        text: '<i class="ti ti-plus me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Aggiungi blog</span>',
            ...
            $(document).on('click', '.delete-record', function () {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            type: 'DELETE',
                            url: "".concat(baseUrl, "blogs/delete/").concat(user_id),
